...
#         self.cursor.execute('''SELECT * FROM drop_table.db''')
...
#         self.cursor.execute('''SELECT from drop_table item_name, item_price, item_quality, item_episode, item_level WHERE monster_name = ?''', (monster_name,))
#         entries = self.cursor.fetchall()
...
#                         item_name, item_quality, item_episode = "N/A"
#                         item_price = -1
#                         item_level = -1
#                         has_drop = 0
...
#                                 break
#                             except ValueError:
#                                 print("Enter a number for the Item Price")
#                                 continue
...
#                                 break
#                             except ValueError:
#                                 print("Enter a number for the Item Level")
#                                 continue

#                         confirm_entry = input(f"\n--- ENTRY CONFIRMATION ---\nMonster Name: '{monster_name}',  Item Name: '{item_name}',  Item Price: '{item_price}',  Item Level: '{item_level}',  Item Quality: '{item_quality}',  Item Episode: '{item_episode}'\n(Y/N): ").strip().upper()
...
#     while True:
#         print("Options:  T - Track Kills  VA - View all kill data  VM - View kill data by monster  Q - Quit program\n=> ")
#         user_input = input().strip().upper()
...
class MonsterDropTable:
    ...
    def view_all_kills(self):
        self.cursor.execute('''SELECT * FROM drop_table''')  # Fixed the query
        entries = self.cursor.fetchall()
        total_entries = 0
    ...
    def view_kills_by_monster(self, monster_name):
        self.cursor.execute('''SELECT * FROM drop_table WHERE monster_name = ?''', (monster_name,))  # Fixed the query
        entries = self.cursor.fetchall()
        kill_count = 0
    ...
    def view_drop_chance_by_quality(self, quality):
        ...
        searched_entries = self.cursor.fetchone()[0] # fetch the first value (total count)
